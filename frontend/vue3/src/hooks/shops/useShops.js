import { getallShops, reqallShops } from "@/api/home";
...
export default function useShops() {
  ...
  const getData = () => {
    ...
    if(keywords.value){
      // 查询
      reqallShops(reqParams.value.categoryId, reqParams.value.page, reqParams.value.pageSize, keywords.value).then((res) => {
        console.log("page:", reqParams.value.page);
        // page=1
        if (reqParams.value.page === 1) {
          result.value = res.result.shops;
      ...
      getallShops(reqParams.value).then((res) => {
        console.log("page:", reqParams.value.page);
        // page=1
        if (reqParams.value.page === 1) {
          result.value = res.result.shops;
